...
function transformFilename(file) {
    ...
    var fileIndex = file.revOrigPath.lastIndexOf("\\");
    var extIndex = file.revOrigPath.lastIndexOf(".");
    var filename = file.revOrigPath.slice(fileIndex + 1, extIndex);
    var extension = file.revOrigPath.slice(extIndex);

    file.history.push(file.revOrigBase + '\\' + filename + '-' + file.revHash + extension);
...
var plugin = function () {
    ...
    return through.obj(function (file, enc, cb) {
        ...
        if (file.isStream()) {
            cb(new gutil.PluginError('gulp-rev-last', 'Streaming not supported'));
            return;
        }
